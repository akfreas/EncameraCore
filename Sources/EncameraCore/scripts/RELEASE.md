...
Release driver for the iOS app. Runs ten preflight gates against App Store Connect, Xcode Cloud and the local repo, then walks the manual ASC steps (localize → attach build → set release type → stage for review → submit → tag).
...
| 6 | The operator confirms at an interactive prompt that the CloudKit `EncMedia` + `EncAlbum` schema (incl. queryable indexes) is deployed to the Production environment | The prompt defaults to No, so a distracted release cannot pass it. CloudKit never auto-creates record types in Production, and a build shipped without the deploy fails every CloudKit write at runtime (`CKError` 12/2006 on the first `saveAlbum`). Deploy via the Dashboard — see `Documentation/cloudkit-schema-deploy.md` — then re-run. |
| 7 | `project.yml` `marketing_version` == the editable ASC version == a VALID TestFlight build | The version being released doesn't line up across the repo, ASC and TestFlight. Reconcile them before releasing. |
| 8 | No TestFlight builds for the release version are in `processingState=PROCESSING` | A build is still being processed by Apple. Wait for it to finish (poll TestFlight or `expire_testflight_builds.py --dry-run` to inspect), then re-run. |
| 9 | The "Build for TestFlight" Xcode Cloud workflow has no `PENDING`/`RUNNING` run | A build is mid-flight on the general workflow and whatever it uploads would supersede the build about to be attached. Wait for it, or cancel it. |
| 10 | The build that will ship was produced by the "Build Release for App Store" workflow, from the commit on HEAD | See below — this is the provenance gate, and the only one that offers to fix itself. |

**Gate 10 — provenance.** The build the release attaches is simply the newest `VALID` TestFlight build for the version, and TestFlight cannot tell you where a build came from: one archived off a feature branch by "Build for TestFlight" (which starts from *any* branch) looks identical there. So provenance is checked at the source. "Build Release for App Store" (`ADD12807-AE85-4814-88C2-F580FFE0C39D`) is manual-start with its source pinned to the `release` branch, and the gate requires that its latest run succeeded, that the run archived the exact commit on HEAD, and that the build that run produced *is* the build that will be attached. Together those mean the binary going to Apple was built from this branch, at this commit, by the one workflow that can't build anything else.

The release workflow is deliberately absent from gate 9, because gate 10 owns its state and can do something better than refuse:
...
`--skip-preflights` bypasses all ten. Use sparingly — these gates exist to catch the exact mistakes that have shipped broken releases in the past.
