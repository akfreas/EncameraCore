...
| `CloudKitFlightCheck.swift` | A manual, end-to-end smoke test (14 ordered steps) that runs the *real* code paths with dummy data against a signed device — entitlement → account → zone → subscription → encrypt+upload → sync → **cold-cache** server download → thumbnail → cancel/restart → delete → album delete cascades to its media. Drives the `ICloudFlightCheckView` workbench behind the `iCloudFlightCheck` toggle. |
...
- **There is no soft-delete state anywhere.** The `deletedAt` field is gone from the schema constants, the metadata types and every read path. A zone written by a soft-deleting build now reads its tombstoned records as live media, which is why the removal waited until the CloudKit plane was still DEBUG-only and no released build had it on. The field itself remains deployed on the Production record types, orphaned, because CloudKit cannot remove a field from a deployed schema.
